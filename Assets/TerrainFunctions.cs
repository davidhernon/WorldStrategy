...
/*
	...
	TerrainFunctions are a library of functions to be computed over 2D worlds of hexagons

	The primary reason for this library was to create the logic to add rivers to our game

	More interesting methods are found near the bottom of the page
...
//				return l;
			...
			for(int i=0; i < x; i++){
				for(int j=0; j < y; j++){
					...
					if(i < area || (i >= x-area)){
						...
						}else if(countNeighborsOfType(terrain, i, j, "shallow_water") > 1 && UnityEngine.Random.Range(0,100) < 40){
							terrain[i,j].type = "grass";
						}else{
	...
	public static double avgValSurroundingTerrain(Hex[,] terrain, int i, int j, int max_x, int max_y){
		double ret = 0.0;
		int count = 0;

		if(i%2==0){

			ret += startTerrainValue(terrain, i, j, max_x, max_y);
			count++;

			if(i!=max_x-1){
				ret += startTerrainValue(terrain, i+1, j, max_x, max_y);
			count++;
			}
			if(i!=0){
				ret += startTerrainValue(terrain, i-1, j, max_x, max_y);
			count++;
				if(j!=0){
					ret += startTerrainValue(terrain, i-1, j-1, max_x, max_y);
			count++;
				}
				if(j!=max_y-1){
					ret += startTerrainValue(terrain, i-1, j+1, max_x, max_y);
			...
			if(j!=0){
				ret += startTerrainValue(terrain, i, j-1, max_x, max_y);
			count++;
			}
			if(j!=max_y-1){
				ret += startTerrainValue(terrain, i, j+1, max_x, max_y);
