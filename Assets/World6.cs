...
public class World6 : MonoBehaviour {
    ...
    void Awake () {
		...
		jungle = new Color[1] {getColor(16,87,16)};
	...
	void Start () {
		...
		// Below is the entirety of the Terrain Generation portion,
		// It is all done within the Start() method and so affects loading times.
		//
	...
	// generates heightmap for Hex heights using Perlin Noise
...
//Builds the list of vertices, colors, and logic for adding triangle faces into a mesh
	void BuildMesh()
	{
	...
	//Currently no logic here, also any changes are slow
	void UpdateMesh()
	{
	...
	//decides between even or odd and calls the BuildHex2 method
	...
	//adds vertex that will be built into the mesh
	void addVertex(Vector3 vert, int x, int y, int pos){
		...
		//ads color for a vertex in the mesh. used by our shader
...
//ads color for a vertex in the mesh. used by our shader. takes a hexes biome into account
	...
	//adds triangle faces for a given hex into our mesh
	//this could be the source of our water height bug but this is not proven yet
...
//used when doing height as distance from edge
...
//initialize hex to water or land
	void initHex(){
...
//counts neighbor hexes of certain type
		...
		public static void addStartLocation(Hex[,] terrain){
			...
			for(int k=0; k < 5; k++){
				...
				while(!satisfied){
					...
					if((farFromCities(k, i, j, 0, 70f) && valueMetricBelowThreshold(distances,d,k,0.05)) || count > 1000){
						satisfied = true;
					}
					count++;
